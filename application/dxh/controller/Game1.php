...
class Game extends Base
{
    ...
    public function history()
    {
        ...
        foreach ($list as $key => $value) {
            $list[$key]['Kjjg'] = explode(',', substr($list[$key]['Kjjg']);
        ...
        foreach ($dxh as $key => $value) {
            if($dxh[$key]['pid'] == 1 || $dxh[$key]['pid'] == 2 || $dxh[$key]['pid'] == 3){
                if($dxh[$key]['val'] == 1 || $dxh[$key]['val'] == 3){
                    $dxh[$key]['odds'] = round($dxh[$key]['odds']+0.05/$level,2);
                }else if($dxh[$key]['val'] == 2){
                    $dxh[$key]['odds'] = round($dxh[$key]['odds']+0.6/$level,2);
                }
            }else if($dxh[$key]['pid'] == 4){
                $dxh[$key]['odds'] = round($dxh[$key]['odds']+0.18/$level,2);
            }else if($dxh[$key]['pid'] == 6){
                $dxh[$key]['odds'] = round($dxh[$key]['odds']+0.08/$level,2);
            }
        }
        $data['odds'] = $dxh;
        echo json_encode(['msg'=>$data,'code'=>1,'success'=>true]);
